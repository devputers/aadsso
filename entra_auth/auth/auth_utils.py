...
ms_settings = settings.ENTRA_CREDS
...
def get_sign_in_flow():
    ...
    return auth_app.initiate_auth_code_flow(scopes=ms_settings["scopes"],
...
def get_token(request):
    ...
    if accounts:
        result = auth_app.acquire_token_silent(ms_settings["scopes"],
                                               account=accounts[0])
        save_cache(request, cache)
...
def validate_email(email):
    is_true = email.split("@")[1] in settings.ENTRA_CREDS["valid_email_domains"]
    return "@" in email and is_true
